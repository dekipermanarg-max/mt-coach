...
export default function PlanningPage() {
  ...
  const [branchId, setBranchId] = useState(""); const [sessions, setSessions] = useState<PlanningRow[]>([]); const [status, setStatus] = useState<"Draft" | "Finalized">("Draft");
  ...
  const [editingId, setEditingId] = useState<string | null>(null); const [modal, setModal] = useState<"draft" | "finalize" | null>(null);
  ...
  async function loadMasters() {
    ...
    if (b.data) setBranchId(b.data.id); setMtRows(mtRes.data || []); setRombelRows(rRes.data || []); setMapelRows(mRes.data || []);
  ...
  function startEdit(session: PlanningRow) {
    ...
    setEditingId(session.id); setMt(mtRows.find(x => x.name === session.mt?.name)?.id || ""); setRombel(rombelRows.find(x => x.name === session.rombel?.name)?.id || ""); setMapel(mapelRows.find(x => x.name === session.mapel?.name)?.id || ""); setType(session.jenis_sesi as (typeof SESSION_TYPES)[number]); setAuviTv(session.auvi_tv); setLd(session.ld); window.scrollTo({ top: 0, behavior: "smooth" });
  }
  function cancelEdit() { setEditingId(null); setMt(""); setRombel(""); setMapel(""); setType("KBM"); setAuviTv(false); setLd(false); }
  async function addOrUpdateSession(e: FormEvent) {
    ...
    const result = editingId ? await supabase.from("weekly_planning").update(payload).eq("id", editingId) : await supabase.from("weekly_planning").insert(payload);
    if (result.error) { setMessage(`Gagal menyimpan sesi: ${result.error.message}`); return; }
    setMessage(editingId ? "Sesi berhasil diperbarui." : "Sesi berhasil ditambahkan ke Draft."); cancelEdit(); await loadSessions();
  }
  ...
  async function saveDraft() { if (!branchId) return; const { error } = await supabase.from("weekly_planning").update({ status: "Draft", updated_at: new Date().toISOString() }).eq("branch_id", branchId).eq("planning_date", date); if (error) { setMessage(`Gagal menyimpan draft: ${error.message}`); return; } setStatus("Draft"); setModal("draft"); loadSessions(); }
  async function finalize() { if (!branchId || sessions.length === 0) return; const { error } = await supabase.from("weekly_planning").update({ status: "Finalized", updated_at: new Date().toISOString() }).eq("branch_id", branchId).eq("planning_date", date); if (error) { setMessage(`Gagal finalisasi: ${error.message}`); return; } setStatus("Finalized"); setModal("finalize"); loadSessions(); }
