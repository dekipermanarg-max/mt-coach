...
function formatDate(value: string) {
  return value ? new Date(`${value}T00:00:00`).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }) : "";
}
...
function adminCount(s: Session) {
  let count = 0;
  if (s.topik_sub_topik_done) count++;
  if (s.attendance) count++;
  if (s.starchamps) count++;
  if (s.activity_score) count++;
  if (s.report_sessions) count++;
  if (s.foto_kbm) count++;
  if (s.report_wa) count++;
  if (s.auvi_tv_status) count++;
  if (s.ld_status) count++;
  return count;
}
...
export default function Performance() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (mtRes.error || branchRes.error || sessionRes.error) setError(mtRes.error?.message || branchRes.error?.message || sessionRes.error?.message || "Gagal memuat data Performance.");
      setMts((mtRes.data || []) as MT[]);
  ...
  const rows = useMemo(() => {
    return mts.map(mt => {
      const own = filtered.filter(s => s.mt_id === mt.id);
      ...
      const admin = planned ? Math.round(own.reduce((sum, s) => sum + adminCount(s), 0) / planned / 9 * 100) : 0;
      const ldOwn = own.filter(s => s.ld_status && s.ld_status !== "Bukan sesi LD");
      const ld = ldOwn.length ? Math.round(ldOwn.filter(s => s.ld_status === "Sudah report di CMS").length / ldOwn.length * 100) : null;
      return { name: mt.name, branch: branchName(mt.branch_id), planned, realized, session, admin, ld };
    }).filter(r => r.planned > 0 && (branch === "Semua Cabang" || r.branch === branch)).sort((a, b) => b.session - a.session || b.admin - a.admin);
  }, [mts, filtered, branches, branch]);

  const avgSession = rows.length ? (rows.reduce((sum, r) => sum + r.session, 0) / rows.length).toFixed(1) : "0.0";
  const avgAdmin = rows.length ? Math.round(rows.reduce((sum, r) => sum + r.admin, 0) / rows.length) : 0;
  const top = rows[0];
  const attention = rows.filter(r => r.session < 90 || r.admin < 90).length;
  ...
  return (
    <div className="performance-shell">
      <main className="performance-main"><div className="performance-wrap">
        ...
        <div className="performance-filters">
          ...
          <label className="branch-filter"><span>Cabang</span><select className="select" value={branch} onChange={e => setBranch(e.target.value)}><option>Semua Cabang</option>{branches.map(b => <option key={b.id}>{b.name}</option>)}</select></label>
        </div>
        ...
        <section className="performance-grid">
          <div className="card performance-kpi"><div className="kpi-label">Average Session</div><div className="kpi-value">{avgSession}%</div><div className="kpi-note">{rows.length} MT dengan sesi Finalized</div></div>
        ...
        <section className="section"><div className="section-head"><div><h2>🏆 Ranking MT</h2><p className="section-note">Ranking mengikuti filter cabang dan rentang tanggal. Sumber data: Monitoring / shared database.</p></div><button className="secondary-btn" type="button">⬇️ Export</button></div>
          <div className="table-wrap"><table><thead><tr><th>#</th><th>MT</th><th>Cabang</th><th>Finalized</th><th>Attendance</th><th>Session</th><th>Admin</th><th>LD</th><th>Status</th></tr></thead><tbody>
            {rows.map((r, i) => <tr key={r.name}><td>{i < 3 ? ["🥇", "🥈", "🥉"][i] : i + 1}</td><td><strong>{r.name}</strong></td><td>{r.branch}</td><td>{r.planned}</td><td>{r.realized}</td><td className="score">{r.session}%</td><td>{r.admin}%</td><td>{r.ld === null ? "—" : `${r.ld}%`}</td><td><span className={`badge ${r.session >= 95 && r.admin >= 95 ? "green" : r.session >= 90 && r.admin >= 90 ? "blue" : "yellow"}`}>{r.session >= 95 && r.admin >= 95 ? "Excellent" : r.session >= 90 && r.admin >= 90 ? "Good" : "Attention"}</span></td></tr>)}
            {rows.length === 0 && !loading && <tr><td colSpan={9}><div className="empty-state">Belum ada sesi Finalized untuk rentang tanggal dan cabang ini.</div></td></tr>}
          </tbody></table></div>
        </section>
        <section className="section"><div className="section-head"><h2>⚠️ Needs Attention</h2></div><div className="attention">{rows.filter(r => r.session < 90 || r.admin < 90).map(r => <div className="alert" key={r.name}><div><strong>{r.name} · Session {r.session}% · Admin {r.admin}%</strong><small>{r.branch} · Session atau administrasi di bawah 90%</small></div><span className="badge yellow">Attention</span></div>)}{attention === 0 && <div className="alert"><div><strong>✅ All good</strong><small>Tidak ada MT yang perlu diperhatikan berdasarkan filter saat ini.</small></div></div>}</div></section>
