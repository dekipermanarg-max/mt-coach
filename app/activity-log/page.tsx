...
function changedFields(row: AuditRow) {
  ...
  return keys
    .filter((key) => !["updated_at"].includes(key) && JSON.stringify(row.old_data?.[key]) !== JSON.stringify(row.new_data?.[key]))
...
export default function ActivityLogPage() {
  ...
  async function load() {
    ...
    const [audit, b, mt, r, m] = await Promise.all([
      supabase.from("audit_log").select("id,table_name,record_id,action,old_data,new_data,changed_at,actor_name,actor_role").order("changed_at", { ascending: false }).limit(500),
        ...
        {loading ? <div className="activity-empty"><strong>Memuat aktivitas…</strong><span>Mohon tunggu sebentar.</span></div> : filtered.length === 0 ? <div className="activity-empty"><strong>Tidak ada aktivitas</strong><span>Coba ubah filter atau kata pencarian.</span></div> : filtered.map((row) => {
          ...
          return <div className="activity-item" key={row.id}>
            ...
            <div className="activity-main">
              <div className="activity-title"><strong>{row.actor_name || "Sistem"}</strong><span className={`action ${row.action.toLowerCase()}`}>{ACTION_LABELS[row.action]}</span><span>{TABLE_LABELS[row.table_name] || row.table_name}</span></div>
              ...
              {isOpen && <div className="activity-detail">
                {row.action === "UPDATE" && changes.length ? changes.map((change) => <div className="activity-detail-row" key={change.key}><strong>{change.key}</strong><span>{valueLabel(change.before)} → <b>{valueLabel(change.after)}</b></span></div>) : <div className="activity-detail-row"><strong>{row.action === "DELETE" ? "Data sebelum dihapus" : "Data"}</strong><span>{recordSummary(row, branches, mts, rombels, mapels)}</span></div>}
              </div>}
            </div>
