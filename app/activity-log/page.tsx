...
export default function ActivityLogPage() {
  ...
  async function load() {
    ...
    setRows((audit.data || []) as AuditRow[]); setBranches((b.data || []) as MasterRow[]); setMts((mt.data || []) as MasterRow[]); setRombels((r.data || []) as MasterRow[]); setMapels((m.data || []) as MasterRow[]); setLoading(false);
  }

  useEffect(() => { if (!authLoading && profile?.role && ["SUPERADMIN", "ATASAN"].includes(profile.role)) void load(); }, [authLoading, profile?.role]);

  const actorLabel = (row: AuditRow) => row.actor_name || row.actor_username || "Sistem";
  const actors = useMemo(() => Array.from(new Set(rows.map((r) => actorLabel(r)).filter(Boolean))).sort((a, b) => a.localeCompare(b)), [rows]);
  ...
  if (!profile || !["SUPERADMIN", "ATASAN"].includes(profile.role)) return <div className="page-wrap"><section className="card" style={{ padding: 28 }}><strong>Akses terbatas.</strong><p style={{ color: "#64748b" }}>Activity Log hanya dapat dilihat oleh Superadmin dan Atasan.</p></section></div>;
