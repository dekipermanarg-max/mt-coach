...
const LD_WEEKLY_TARGET_PERCENT = 50;
...
export default function Home() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [b, mt, s] = await Promise.all([
  ...
  const runningRombels = new Set(visibleSessions.map(s => s.rombel_id).filter(Boolean)).size;
  const auviSessions = visibleSessions.filter(s => s.auvi_tv_status === "Connect ke TV" || s.auvi_tv).length;
  const auviProgress = Math.min(100, Math.round((auviSessions / AUVI_WEEKLY_TARGET) * 100));
  const ldRombels = new Set(visibleSessions.filter(s => s.ld && s.rombel_id).map(s => s.rombel_id)).size;
  const ldGoal = Math.ceil(runningRombels * LD_WEEKLY_TARGET_PERCENT / 100);
  const ldProgress = ldGoal ? Math.min(100, Math.round((ldRombels / ldGoal) * 100)) : 0;
  ...
  return (
    <div className="page-wrap dashboard-page">
      ...
      {loading ? <div className="card"><div className="kpi-note">Memuat data Monitoring...</div></div> : <>
        <section className="grid"><div className="card"><div className="kpi-label">Session Completion</div><div className="kpi-value">{sessionCompletion}%</div><div className="kpi-note">{realized} / {planned} realized</div></div><div className="card"><div className="kpi-label">AuVi TV Weekly Target</div><div className="kpi-value">{auviSessions} / {AUVI_WEEKLY_TARGET}</div><div className="kpi-note">Target {AUVI_WEEKLY_TARGET} sesi / minggu · {auviProgress}%</div></div><div className="card"><div className="kpi-label">LD Weekly Target</div><div className="kpi-value">{ldRombels} / {ldGoal}</div><div className="kpi-note">≥ {LD_WEEKLY_TARGET_PERCENT}% unique rombel berjalan · {ldProgress}%</div></div><div className="card"><div className="kpi-label">Active MT</div><div className="kpi-value">{activeMTs.length}</div><div className="kpi-note">{branch === "all" ? "Total semua cabang" : "MT dengan base di cabang ini"}</div></div></section>
