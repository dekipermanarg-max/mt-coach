      ...
      {loading ? <div className="card"><div className="kpi-note">Memuat data Monitoring...</div></div> : <>
        ...
        <section className="section"><div className="section-head"><h2>⚠️ Needs Attention</h2></div>
          <div className="grid attention-grid">
            <div className="card attention-card"><div className="section-head"><div><h3>👤 MT</h3><div className="kpi-note">Session completion di bawah 90%</div></div><Link className="badge yellow" href="/performance">Lihat Performance →</Link></div>{mtAttention.length ? <div className="attention">{mtAttention.map(r => <div className="alert" key={r.id}><div><strong>{r.name}</strong><small>Session completion {r.score}% · {r.planned - r.realized} sesi belum realized</small></div></div>)}</div> : <div className="alert"><div><strong>✅ All good</strong><small>Tidak ada MT dengan session completion di bawah 90%.</small></div></div>}</div>
