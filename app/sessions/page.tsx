...
export default function SessionsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return sessions.filter((s) => {
      ...
      const matchesSearch = !q || [branch, mt, product, rombel].some((x) => x.toLowerCase().includes(q));
      ...
      return matchesDateFrom && matchesDateTo && matchesSearch && matchesBranch && (filterProduct === "all" || s.program_id === filterProduct);
    });
  ...
  const mtAttention = useMemo(() => {
    ...
    filtered.forEach((s) => {
      ...
      const row = map.get(s.mt_id) || { name, total: 0, incomplete: 0, attendance: 0, notes: 0 };
      row.total++;
      if (!s.is_complete) row.incomplete++;
      if (s.attendance) row.attendance++;
      if (s.teacher_notes) row.notes++;
      map.set(s.mt_id, row);
    });
  ...
  const branchAttention = useMemo(() => {
    ...
    filtered.forEach((s) => {
      const name = branches.find((x) => x.id === s.branch_id)?.name || "—";
      const row = map.get(s.branch_id) || { name, total: 0, incomplete: 0 };
      row.total++;
      if (!s.is_complete) row.incomplete++;
      map.set(s.branch_id, row);
    });
    return [...map.values()].filter((x) => x.incomplete > 0).sort((a, b) => b.incomplete - a.incomplete || a.name.localeCompare(b.name));
  }, [filtered, branches]);
  ...
  const rombelAttention = useMemo(() => {
    const map = new Map<string, { name: string; total: number; incomplete: number }>();
    filtered.forEach((s) => {
      const name = MATHCHAMPS_ROMBELS.find((x) => x.id === s.rombel_id)?.name || "—";
      const row = map.get(s.rombel_id) || { name, total: 0, incomplete: 0 };
      row.total++;
      if (!s.is_complete) row.incomplete++;
      map.set(s.rombel_id, row);
    });
    return [...map.values()].filter((x) => x.incomplete > 0).sort((a, b) => b.incomplete - a.incomplete || a.name.localeCompare(b.name));
  }, [filtered]);
  ...
  function openAddForm() {
    setEditingId(null);
    setForm({ ...emptyForm, branch_id: branches[0]?.id || "", mt_id: mts[0]?.id || "" });
  ...
  function openEditForm(session: SessionRow) {
    ...
    setAdmin({ attendance: session.attendance, teacher_notes: session.teacher_notes });
    setMessage("");
    setShowForm(true);
  }
  ...
  async function saveSession(e: FormEvent) {
    ...
    if (editingId) {
      ...
      if (error) {
        setMessage(`Gagal mengubah sesi: ${error.message}`);
        setSaving(false);
        return;
      }

      const { error: adminError } = await supabase.from("session_admin").upsert({
      ...
      if (adminError) {
        setMessage(`Sesi sudah diubah, tetapi administrasi gagal diperbarui: ${adminError.message}`);
        setSaving(false);
        await load();
        return;
      }
      ...
      const { error: adminError } = await supabase.from("session_admin").insert({
  ...
  async function deleteSession(session: SessionRow) {
    const branch = label(session.branch_id, branches);
    const mt = label(session.mt_id, mts);
    const product = productLabel(session.program_id);
    const rombel = label(session.rombel_id, MATHCHAMPS_ROMBELS);
    ...
    setMessage("");
    const { error: adminError } = await supabase.from("session_admin").delete().eq("session_id", session.id);
    if (adminError) {
      setMessage(`Gagal menghapus administrasi sesi: ${adminError.message}`);
      return;
    }
    ...
    setMessage("Sesi Mathchamps berhasil dihapus.");
    await load();
  }
    ...
    <div className="page-wrap">
      <section className="planning-hero">
        <div className="planning-hero-row">
          ...
          <button type="button" className="primary-btn" onClick={openAddForm}>＋ Tambah Sesi</button>
        </div>
      </section>

      <div className="product-filter-row">
        <div className="product-strip">
          <button type="button" className={filterProduct === "all" ? "product-pill active" : "product-pill"} onClick={() => setFilterProduct("all")}>Semua Produk</button>
          {MATHCHAMPS_PROGRAMS.map((p) => <button type="button" key={p.id} className={filterProduct === p.id ? "product-pill active" : "product-pill"} onClick={() => setFilterProduct(p.id)}>{p.name}</button>)}
        </div>
        <div className="period-filter">
          <span className="period-label">Periode</span>
          <input type="date" aria-label="Tanggal mulai" value={filterDateFrom} onChange={(e) => setFilterDateFrom(e.target.value)} />
          <span className="period-separator">s.d.</span>
          <input type="date" aria-label="Tanggal selesai" value={filterDateTo} min={filterDateFrom || undefined} onChange={(e) => setFilterDateTo(e.target.value)} />
          {(filterProduct !== "all" || filterBranch !== "all" || filterDateFrom || filterDateTo || search) && <button type="button" className="clear-filter-btn" onClick={clearFilters}>Reset</button>}
        </div>
      </div>

      <div className="secondary-filter-row">
        <label className="branch-filter"><span>Cabang</span><select value={filterBranch} onChange={(e) => setFilterBranch(e.target.value)}><option value="all">Semua Cabang</option>{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label>
      </div>
      ...
      <section className="card session-panel">
        <div className="session-toolbar">
          <div><h2>Daftar Sesi Mathchamps</h2><p>Sesi <b>Lengkap</b> jika <b>Attendance + Teacher Notes</b> keduanya terisi.</p></div>
          <div className="session-filters"><input className="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari cabang, MT, produk..." /></div>
        </div>
        ...
        <div className="table-wrap session-table-wrap">
          <table>
            <thead><tr><th>Tanggal</th><th>Jam</th><th>Cabang</th><th>MT</th><th>Produk</th><th>Rombel</th><th>Attendance</th><th>Teacher Notes</th><th>Status</th><th>Aksi</th></tr></thead>
            <tbody>
              {filtered.map((s) => (
                <tr key={s.id}>
                  ...
                  <td><div className="row-actions"><button type="button" className="edit-btn" onClick={() => openEditForm(s)}>Edit</button><button type="button" className="delete-btn" onClick={() => deleteSession(s)}>Hapus</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="attention-section">
        <div className="section-head"><div><h2>⚠️ MT Need Attention</h2><p className="section-note">MT yang memiliki minimal satu sesi belum lengkap pada filter saat ini.</p></div><span className="attention-count">{mtAttention.length} MT</span></div>
        {mtAttention.length ? <div className="attention-grid">{mtAttention.map((r) => <div className="attention-card" key={r.name}><div className="attention-card-top"><strong>{r.name}</strong><span className="warning-badge">{r.incomplete} belum lengkap</span></div><div className="attention-meta">{r.total} sesi · Attendance {r.attendance}/{r.total} · Notes {r.notes}/{r.total}</div><div className="attention-bar"><span style={{ width: `${Math.round(r.incomplete / r.total * 100)}%` }} /></div></div>)}</div> : <div className="all-good">✅ Semua MT pada filter ini sudah lengkap.</div>}
      </section>

      <div className="attention-two-col">
        <section className="attention-section compact"><div className="section-head"><div><h2>🏢 Cabang Need Attention</h2><p className="section-note">Cabang dengan sesi belum lengkap.</p></div></div>{branchAttention.length ? <div className="mini-list">{branchAttention.map((r) => <div className="mini-row" key={r.name}><div><strong>{r.name}</strong><small>{r.total} sesi</small></div><span>{r.incomplete} belum lengkap</span></div>)}</div> : <div className="all-good">✅ Semua cabang aman.</div>}</section>
        <section className="attention-section compact"><div className="section-head"><div><h2>👥 Rombel Need Attention</h2><p className="section-note">Rombel dengan sesi belum lengkap.</p></div></div>{rombelAttention.length ? <div className="mini-list">{rombelAttention.map((r) => <div className="mini-row" key={r.name}><div><strong>{r.name}</strong><small>{r.total} sesi</small></div><span>{r.incomplete} belum lengkap</span></div>)}</div> : <div className="all-good">✅ Semua rombel aman.</div>}</section>
      </div>
      ...
      {showForm && (
        <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) closeForm(); }}>
          <form className="session-modal" onSubmit={saveSession}>
            <div className="modal-head"><div><div className="eyebrow">{editingId ? "EDIT MATHCHAMPS SESSION" : "NEW MATHCHAMPS SESSION"}</div><h2>{editingId ? "Edit Sesi" : "Tambah Sesi"}</h2><p>Masukkan detail sesi dan administrasinya.</p></div><button type="button" className="close-btn" onClick={closeForm}>×</button></div>
            <div className="form-grid">
              <label>Tanggal<input type="date" value={form.session_date} onChange={(e) => setField("session_date", e.target.value)} required /></label>
              <label>Mulai<input type="time" value={form.start_time} onChange={(e) => setField("start_time", e.target.value)} required /></label>
              <label>Selesai<input type="time" value={form.end_time} onChange={(e) => setField("end_time", e.target.value)} required /></label>
              <label>Cabang<select value={form.branch_id} onChange={(e) => setField("branch_id", e.target.value)} required><option value="">Pilih cabang</option>{branches.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
              <label>Nama MT<select value={form.mt_id} onChange={(e) => setField("mt_id", e.target.value)} required><option value="">Pilih MT</option>{mts.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
              <label>Produk<select value={form.program_id} onChange={(e) => setField("program_id", e.target.value)} required><option value="">Pilih produk</option>{MATHCHAMPS_PROGRAMS.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
              <label>Rombel<select value={form.rombel_id} onChange={(e) => setField("rombel_id", e.target.value)} required><option value="">Pilih rombel</option>{MATHCHAMPS_ROMBELS.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
            </div>
            <div className="admin-box"><div><b>Administrasi Mathchamps</b><span>Keduanya harus terisi agar sesi berstatus Lengkap.</span></div><label className="check-row"><input type="checkbox" checked={admin.attendance} onChange={(e) => setAdmin((a) => ({ ...a, attendance: e.target.checked }))} /><span>Attendance</span></label><label className="check-row"><input type="checkbox" checked={admin.teacher_notes} onChange={(e) => setAdmin((a) => ({ ...a, teacher_notes: e.target.checked }))} /><span>Teacher Notes</span></label></div>
            <div className="modal-actions"><button type="button" className="secondary-btn" onClick={closeForm} disabled={saving}>Batal</button><button type="submit" className="primary-btn" disabled={saving}>{saving ? "Menyimpan…" : editingId ? "Simpan Perubahan" : "Simpan Sesi"}</button></div>
          </form>
        </div>
      )}
      ...
      <style jsx>{`
        ...
        .session-panel{padding:18px}.session-toolbar{display:flex;justify-content:space-between;gap:16px;align-items:flex-end;margin-bottom:16px}.session-toolbar h2{margin:0;color:#172033;font-size:19px}.session-toolbar p{margin:5px 0 0;color:#64748b;font-size:12px}.session-filters{display:flex;gap:8px}.session-filters input{height:40px;border:1px solid #d8e0ea;border-radius:10px;padding:0 11px;background:#f8fafc;font-size:12px}.session-filters .search{width:220px}.session-table-wrap{overflow:auto}.session-table-wrap table{min-width:1160px}.session-table-wrap input[type=checkbox]{width:17px;height:17px;accent-color:#2563eb;cursor:pointer}.program-badge{display:inline-flex;padding:5px 8px;border-radius:7px;background:#f1f5f9;color:#334155;font-size:10px;font-weight:800}.status-complete,.status-incomplete{font-size:11px;font-weight:800;white-space:nowrap}.status-complete{color:#16a34a}.status-incomplete{color:#d97706}.notice{margin-bottom:12px;padding:11px 13px;border-radius:10px;background:#eff6ff;color:#1d4ed8;font-size:12px}.empty-state{text-align:center;padding:34px;color:#94a3b8;font-size:12px}.row-actions{display:flex;gap:6px}.edit-btn,.delete-btn{height:31px;padding:0 9px;border-radius:8px;font-size:11px;font-weight:800;cursor:pointer}.edit-btn{border:1px solid #bfdbfe;background:#eff6ff;color:#2563eb}.delete-btn{border:1px solid #fecaca;background:#fff5f5;color:#dc2626}
        .attention-section{margin-top:18px;background:#fff;border:1px solid #e7ebf1;border-radius:14px;padding:18px}.section-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:14px}.section-head h2{margin:0;font-size:18px;color:#172033}.section-note{margin:5px 0 0;color:#64748b;font-size:12px}.attention-count{font-size:11px;font-weight:800;color:#b45309;background:#fff7ed;border:1px solid #fed7aa;padding:6px 9px;border-radius:999px}.attention-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.attention-card{border:1px solid #edf0f4;border-radius:12px;padding:13px;background:#fff}.attention-card-top{display:flex;justify-content:space-between;align-items:center;gap:10px}.attention-card-top strong{font-size:13px;color:#172033}.warning-badge{font-size:10px;font-weight:800;color:#b45309;background:#fff7ed;border-radius:999px;padding:5px 7px;white-space:nowrap}.attention-meta{margin-top:7px;font-size:11px;color:#64748b}.attention-bar{height:5px;background:#f1f5f9;border-radius:999px;margin-top:10px;overflow:hidden}.attention-bar span{display:block;height:100%;background:#f59e0b;border-radius:999px}.all-good{padding:15px;border-radius:10px;background:#f0fdf4;color:#15803d;font-size:12px;font-weight:700}.attention-two-col{display:grid;grid-template-columns:1fr 1fr;gap:18px}.attention-two-col .attention-section{margin-top:18px}.mini-list{display:flex;flex-direction:column}.mini-row{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:11px 0;border-bottom:1px solid #eef2f6}.mini-row:last-child{border-bottom:0}.mini-row>div{display:flex;flex-direction:column;gap:3px}.mini-row strong{font-size:12px;color:#172033}.mini-row small{font-size:10px;color:#94a3b8}.mini-row>span{font-size:10px;font-weight:800;color:#b45309;background:#fff7ed;padding:5px 7px;border-radius:999px;white-space:nowrap}
        ...
        @media(max-width:620px){.session-kpis{grid-template-columns:1fr 1fr}.session-filters{flex-direction:column}.form-grid{grid-template-columns:1fr}.session-modal{padding:18px;border-radius:16px}.admin-box .check-row{display:flex;margin:9px 0}.period-filter input{flex:1;min-width:120px}.branch-filter{align-items:stretch;flex-direction:column;gap:6px}.branch-filter select{width:100%}}
      `}</style>
    </div>
  );
}
