...
export default function SessionsPage() {
  ...
  // Satu baris = satu kombinasi Rombel + Cabang + Produk yang masih punya sesi belum lengkap.
  ...
  return <div className="page-wrap">
    ...
    <div className="attention-two-col"><section className="attention-section compact"><div className="section-head"><div><h2>🏢 Cabang Need Attention</h2><p className="section-note">Cabang dengan sesi belum lengkap.</p></div></div>{branchAttention.length ? <div className="mini-list">{branchAttention.map(r => <div className="mini-row" key={r.name}><div><strong>{r.name}</strong><small>{r.total} sesi</small></div><span>{r.incomplete} belum lengkap</span></div>)}</div> : <div className="all-good">✅ Semua cabang aman.</div>}</section>
      <section className="attention-section compact"><div className="section-head"><div><h2>👥 Rombel Need Attention</h2><p className="section-note">Menampilkan rombel, cabang, dan produk yang memiliki sesi belum lengkap.</p></div></div>{rombelAttention.length ? <div className="rombel-list">{rombelAttention.map(r => <div className="rombel-row" key={`${r.rombel}-${r.branch}-${r.product}`}><div className="rombel-main"><strong>{r.rombel}</strong><span>{r.branch}</span><span className="program-badge">{r.product}</span></div><div className="rombel-count"><strong>{r.incomplete}</strong><small>belum lengkap</small></div></div>)}</div> : <div className="all-good">✅ Semua rombel aman.</div>}</section></div>
    ...
    <style jsx>{`
      ...
      @media(max-width:620px){.session-kpis{grid-template-columns:1fr 1fr}.session-filters{flex-direction:column}.form-grid{grid-template-columns:1fr}.session-modal{padding:18px;border-radius:16px}.admin-box .check-row{display:flex;margin:9px 0}.period-filter input{flex:1;min-width:120px}.branch-filter{align-items:stretch;flex-direction:column;gap:6px}.branch-filter select{width:100%}.rombel-row{align-items:flex-start}.rombel-main{flex-direction:column;align-items:flex-start;gap:5px}.rombel-count{margin-left:auto}}
    `}</style>
  </div>;
}
