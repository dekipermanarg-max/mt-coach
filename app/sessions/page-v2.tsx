...
export default function SessionsPageV2() {
  ...
  const formMts = useMemo(() => mts.filter(mt => mt.branch_id === form.branch_id), [mts, form.branch_id]);
  ...
  function changeBranch(branchId: string) {
    const branchMts = mts.filter(mt => mt.branch_id === branchId);
    setForm(current => ({ ...current, branch_id: branchId, mt_id: branchMts[0]?.id || "" }));
  }
  ...
  function openAddForm() {
    ...
    const branchMts = mts.filter(mt => mt.branch_id === branchId);
    setEditingId(null);
    setForm({ ...EMPTY, branch_id: branchId, mt_id: branchMts[0]?.id || "" });
  ...
  async function saveSession(e: FormEvent) {
    ...
    if (!formMts.some(mt => mt.id === form.mt_id)) { setMessage("MT yang dipilih harus sesuai dengan cabang sesi."); return; }
