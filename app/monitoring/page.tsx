...
export default function Monitoring() {
  ...
  async function load() {
    ...
    if (error) {
      console.error("[Monitoring] weekly_planning Finalized query failed:", error);
      setRows([]);
      setStartDate("");
      setEndDate("");
      ...
      if (nextRows.length) {
        ...
        setStartDate("");
        setEndDate("");
        setMessage("Tidak ada sesi Finalized yang dapat diakses akun ini. Jika sesi sudah Finalized di Weekly Planning, cek akses cabang akun.");
      }
    }
    setLoading(false);
  }
      ...
      <div className="monitoring-cards">
        {loading ? <div className="empty-state"><strong>Memuat data…</strong></div> : filtered.length === 0 ? <div className="empty-state"><div className="empty-icon">📋</div><strong>Belum ada sesi Finalized</strong><p>Sesuaikan tanggal atau filter untuk melihat sesi.</p></div> : filtered.map(row => {
          ...
          return <article key={row.id} className={`monitoring-session-card ${complete ? "is-complete" : "is-incomplete"}`}>
            ...
            {open && <div className="monitoring-card-body"><div className="monitoring-admin-title"><span>KELENGKAPAN ADMINISTRASI</span><div className="monitoring-admin-title-actions"><span>{done}/{total} selesai</span>{!complete && <button type="button" className="complete-all-btn" disabled={saving === row.id} onClick={() => completeAllCheckboxes(row)}>✓ Semua Lengkap</button>}</div></div>{simple && <div className="card" style={{ marginBottom: 12, padding: 10, fontSize: 12, background: "#f8fafc" }}>ℹ️ Untuk {row.jenis_sesi}, cukup <strong>Attendance</strong> yang wajib diisi. Administrasi lainnya tidak memengaruhi status kelengkapan.</div>}<div className="monitoring-admin-grid">{checkboxItems.filter(([key]) => !simple || key === "attendance").map(([key, label]) => <label key={key} className={`admin-item ${row[key] ? "done" : "todo"}`}><input type="checkbox" checked={row[key]} disabled={saving === row.id} onChange={e => saveRow(row, { [key]: e.target.checked })} /><span>{label}</span><b>{row[key] ? "✓" : "—"}</b></label>)}{!simple && <><label className={`admin-item select-item ${isAuviComplete(row) ? "done" : "todo"}`}><span>📺 AuVi TV</span><select className="select monitoring-select" value={row.auvi_tv_status || ""} disabled={saving === row.id} onChange={e => saveRow(row, { auvi_tv_status: e.target.value })}><option value="">— Pilih status —</option>{AUVISTATUSES.map(x => <option key={x}>{x}</option>)}</select></label><label className={`admin-item select-item ${isLdComplete(row) ? "done" : "todo"}`}><span>👥 LD</span><select className="select monitoring-select" value={row.ld_status || ""} disabled={saving === row.id} onChange={e => saveRow(row, { ld_status: e.target.value })}><option value="">— Pilih status —</option>{LDSTATUSES.map(x => <option key={x}>{x}</option>)}</select></label></>}</div><div className="monitoring-save-note">{saving === row.id ? "Menyimpan perubahan…" : "Perubahan tersimpan otomatis ke shared database."}</div></div>}
          </article>;
        })}
      </div>
    </section>
