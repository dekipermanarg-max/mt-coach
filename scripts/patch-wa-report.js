...
if (start === -1 || end === -1) throw new Error("WA report block marker not found");

const replacement = `  const waReport = useMemo(() => {
    ...
    if (!reportIncompleteRows.length) {
      lines.push(
        "Terima kasih sudah melengkapi seluruh administrasi KBM/Klinik PR kemarin. 🙌",
        "",
        "Apresiasi untuk kerja sama, tanggung jawab, dan komitmen teman-teman dalam menjaga administrasi tetap tertib. 👏",
        "",
        "Semoga konsisten terus ke depannya ya!",
    ...
    Array.from(byMt.entries()).sort((a, b) => nameOf(mts, a[0]).localeCompare(nameOf(mts, b[0]))).forEach(([mtId, mtRows]) => {
      mtRows.forEach(row => {
        ...
        lines.push(
          `• *${nameOf(mts, mtId)}* — ${nameOf(rombels, row.rombel_id)} — ${nameOf(mapels, row.mapel_id)}`,
          `  ➜ ${missing.join(" · ")}`,
          "",
        );
      });
    ...
    lines.push(
      "Jika sudah dilengkapi, mohon *react ✅ pada pesan ini* sebagai konfirmasi.",
      "",
      "Terima kasih atas kerja sama dan komitmennya.",
      "Semangat mengajar hari ini, teman-teman! 💙🔥",
    );
    return lines.join("\\n");
  }, [reportIncompleteRows, waDate, mts, rombels, mapels]);`;

s = s.slice(0, start) + replacement + s.slice(end);
fs.writeFileSync(file, s);
console.log("Updated WhatsApp report wording:", file);
