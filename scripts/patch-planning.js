...
const replacements = [
  ...
  ['          <label className="planning-field"><span>MT</span><select value={mt} onChange={(e) => setMt(e.target.value)} disabled={loading} required>{mtRows.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>', '          <label className="planning-field"><span>MT</span><select value={mt} onChange={(e) => setMt(e.target.value)} required><option value="">Pilih MT...</option>{mtRows.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>'],
  ['          <label className="planning-field"><span>Rombel</span><select value={rombel} onChange={(e) => setRombel(e.target.value)} disabled={loading} required>{rombelRows.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>', '          <label className="planning-field"><span>Rombel</span><select value={rombel} onChange={(e) => setRombel(e.target.value)} required><option value="">Pilih rombel...</option>{rombelRows.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>'],
  ['          <label className="planning-field"><span>Mapel</span><select value={mapel} onChange={(e) => setMapel(e.target.value)} disabled={loading} required>{mapelRows.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>', '          <label className="planning-field"><span>Mapel</span><select value={mapel} onChange={(e) => setMapel(e.target.value)} required><option value="">Pilih mapel...</option>{mapelRows.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>'],
...
// MT is a shared pool across all branches: an MT may teach at a branch different from their base branch.
// Rombel and mapel remain shared master data.
...
console.log("Patched Weekly Planning: dropdowns stay interactive; all active MTs are available across all branches; rombel/mapel remain shared; saved-session names hydrated:", file);
