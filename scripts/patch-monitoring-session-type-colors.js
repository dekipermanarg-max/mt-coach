...
const currentSubmeta = s.includes(oldSubmeta) ? oldSubmeta : s.match(/<div className="monitoring-submeta">\{nameOf\(rombels, row\.rombel_id\).*?<\/div>/)?.[0];
if (!currentSubmeta) throw new Error("monitoring session type label marker not found");
s = s.replace(currentSubmeta, oldSubmetaPatched);

fs.writeFileSync(file, s);
console.log("Applied Monitoring session-type colors: KBM red, Klinik PR blue, Trial Class yellow");
